...
contract TrendFairlaunchPoolV2 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable{
...
//  mapping
    ...
    event TokensDebt(address indexed holder, uint256 ethAmount);
...
function contribute(address _sponser) payable external {
        ...
        if (saleInfo.maxPay > 0) {
            require(msg.value <= saleInfo.maxPay, "Exceeds your Max Pay");
            require(user.totalInvestedETH + msg.value <= saleInfo.maxPay, "Exceeds your Max Pay");
        }
        totalInvestedETH = totalInvestedETH +msg.value;
        user.totalInvestedETH = user.totalInvestedETH +msg.value;
        user.debt=user.debt+ msg.value;
        ...
        _handleAffiliate(_sponser, msg.value); 
        }
          emit TokensDebt(msg.sender, msg.value);
    }
       /**
...
function getTokenPrice(uint256 _userContribution) public view returns (uint256) {
    return totalInvestedETH > 0 ? _userContribution * (saleInfo.tokenAmount)/(totalInvestedETH): saleInfo.tokenAmount;    
}

// Calculate current tokens Rate  
function currentTokenRate() public view returns (uint256) {
    if (totalInvestedETH == 0) return 0;
    return ((saleInfo.tokenAmount) * (10 ** 18)) / (totalInvestedETH);
}


// public Function 1×10²⁷
...
function finalize() external payable onlyOwner {
    ...
    if (saleInfo.lpPercent > 0) {
        ...
        uint256 ethForLp = contributionRemain * (saleInfo.lpPercent)/(100);
        contributionRemain = contributionRemain - ethForLp;
        ...
        (,, uint256 liquidity) = uniswapRouter.addLiquidityETH{value: ethForLp}(
        ...
        // Lock LP Tokens
        address lpTokenAddress = IUniswapV2Factory(dexInfo.factory).getPair(address(saleInfo.saleToken), dexInfo.weth);
    ...
    if (saleInfo.isBuyBackEnabled) {
        uint256 buybackAmount = (totalInvestedETH * buyBackInfo.buybackPercentage)/100;
        buybackAmount = buybackAmount - (buybackAmount * feePercent)/100;
        contributionRemain = contributionRemain - buybackAmount;
        IBuyBackManager(buyBackInfo.buyBackMangerAddrress).finalizeBuyBackConfig{value: buybackAmount}(address(this),buybackAmount);
    }
    ...
    function RefundToContributor() external onlyOwner {
            ...
            for (uint256 i = 0; i < participants.length; i++) {
                ...
                if (user.totalInvestedETH > 0 && !user.isRefunded) {
                    uint256 amount = user.totalInvestedETH;
                    user.totalInvestedETH = 0; // Reset investment
                    user.debt = 0; // Reset debt
                    user.isRefunded = true; // Mark as refunded
                    (bool success, ) = participants[i].call{value: amount}("");
...
function enableAffilate(bool _enabled, uint8 _rate) public onlyOwner {
    require(
